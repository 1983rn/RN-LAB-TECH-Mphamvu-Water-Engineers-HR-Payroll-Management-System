...
def list_invoices():
    query = Invoice.query
    invoices = apply_dept_filter(query, Invoice).order_by(Invoice.created_at.desc()).all()
    # Fetch approved quotations that haven't had an invoice generated yet
    q_query = Quotation.query.filter_by(status='Approved', invoice_generated=False)
    approved_quotations = apply_dept_filter(q_query, Quotation).all()
    return render_template('accounts/invoices/list.html', invoices=invoices, approved_quotations=approved_quotations)
...
def download_invoice_pdf(invoice_id):
    ...
    story = add_company_header_to_story(story, layout_mode=layout_mode, department=invoice.department)
...
def list_delivery_notes():
    ...
    delivery_notes = apply_dept_filter(query, DeliveryNote).order_by(DeliveryNote.created_at.desc()).all()
    
    # Fetch invoices that are approved but haven't had a delivery note generated yet
    i_query = Invoice.query.filter_by(is_approved=True)
    approved_invoices = apply_dept_filter(i_query, Invoice).all()
    ...
    return render_template('accounts/delivery/list.html', 
...
def approve_invoice():
    ...
    try:
        ...
        return jsonify({
            'success': True, 
            'message': 'Invoice Approved and Delivery Note generated successfully!'
        })
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'message': str(e)}), 500
...
def generate_delivery_note_from_invoice():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'success': False, 'message': str(e)}), 500
@finance_bp.route('/receipt/<int:transaction_id>/download')
@login_required
def download_receipt(transaction_id):
    transaction = Transaction.query.get_or_404(transaction_id)
    
    try:
        buffer = generate_receipt_pdf(transaction)
        
        response = make_response(buffer.getvalue())
        response.headers['Content-Type'] = 'application/pdf'
        response.headers['Content-Disposition'] = f'inline; filename=Receipt_{transaction.transaction_id}.pdf'
        return response
    except Exception as e:
